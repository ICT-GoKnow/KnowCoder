    ...
    for idx, data in enumerate(tqdm(dataset)):
        ...
        for event in data["events"]:
            ...
            if norm_name(event["type"]) in event_dict:
                e = {"type": event_dict[norm_name(event["type"])], "trigger": {"word": event["trigger"]}, "args": label_args}
                #e = {"type": event_dict[norm_name(event["type"])], "trigger": {"word":" "}, "args": label_args}
            else:
                e = {"type": norm_name(event["type"]), "trigger": {"word": event["trigger"]}, "args": label_args}
        ...
        for event_type, event_t_a in events_dict:
            ...
            e = {"type": event_type, "trigger": {"word": trigger}, "args": {}}
